	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`shezhang`, this.ruleForm.id);
...
//跨表计算判断
				if((!this.ruleForm.shezhangzhanghao)){
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`shezhang`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`shezhang`, this.ruleForm);
						}else{
							await this.$api.add(`shezhang`, this.ruleForm);
						}
