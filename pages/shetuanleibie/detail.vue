...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('shetuanleibie', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussshetuanleibie/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('shetuanleibie', this.detail);
				this.$utils.msg('审核成功');
